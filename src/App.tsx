        ...
        <main className="flex-1 overflow-y-auto p-3 md:p-6 lg:p-8">
          {selectedDoc ? (
            <div className={`max-w-4xl mx-auto ${t.cardBg} rounded-xl shadow-lg p-4 md:p-6 lg:p-8`}>
              ...
              <div className="mb-6 flex flex-wrap gap-4 items-end bg-black/5 p-4 rounded-xl border border-black/10 dark:bg-white/5 dark:border-white/10">
                <div className="flex flex-col gap-1.5 flex-1 min-w-[250px]">
                  <span className={`text-xs font-bold uppercase tracking-wider ${t.headerSubtext}`}>Áudio Offline (Edge TTS)</span>
                  <audio 
                    controls 
                    src={`/audios/${selectedDoc.id}.mp3`} 
                ...
                <a 
                  href={`/audios/${selectedDoc.id}.mp3`} 
                  download={`${selectedDoc.title}.mp3`}
                  className={`px-4 py-2.5 h-10 text-sm font-medium rounded-lg bg-blue-600 text-white hover:bg-blue-700 flex items-center justify-center gap-2 transition-all shadow-sm flex-shrink-0`}
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
